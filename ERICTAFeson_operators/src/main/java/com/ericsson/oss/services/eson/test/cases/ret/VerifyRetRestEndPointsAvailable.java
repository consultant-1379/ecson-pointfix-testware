...
public class VerifyRetRestEndPointsAvailable extends TafTestBase {

    private static final String TEST_ID = "SONP-32623_VerifyRestEndPointsAvailable";
    ...
    @TestSuite
    @TestId(id = "SONP-32625_ScheduleRetAndVerifyProposedChange", title = "eSON : SONP-32625_ScheduleRetAndVerifyProposedChange - Verify REST Endpoints are working using default ingress.")
    ...
    public void setUpServerForEnv() {
        final TestScenario scenario = dataDrivenScenario("set up server for EC-SON set default ingress")
                ...
                .withScenarioDataSources(dataSource(ENVIRONMENT_FILE)
                        .withFilter(String.format(FILTER_BY_TESTCASE_ID, "SONP-32625_ScheduleRetAndVerifyProposedChange")))
                .build();
        CommonUtil.start(scenario);
    }

    @TestSuite
    @TestId(id = TEST_ID, title = "eSON : SONP-32623_VerifyRestEndPointsAvailable - Verify REST Endpoints are working using default ingress.")
